...
const BorderGlow = ({
  ...
  useEffect(() => {
    ...
    if (animated) {
      ...
      animateValue({ duration: 500, onUpdate: (v: number) => card.style.setProperty('--edge-proximity', `${v.toFixed(3)}`) });
      ...
      animateValue({ ease: easeInCubic, delay: 2500, duration: 1500, start: 100, end: 0,
        onUpdate: (v: number) => card.style.setProperty('--edge-proximity', `${v.toFixed(3)}`),
    ...
    return () => card.removeEventListener('pointermove', handlePointerMove as any);
  }, [animated, handlePointerMove]);
